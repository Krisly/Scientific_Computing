...
figure
plot(stepRange, stencil1Output, stepRange, ones(size(stepRange)).* F2d(x), 'LineWidth', 1.5)
ylabel("f''(pi/2)")
xlabel("step size")
...
figure
loglog(stepRange, abs(ones(size(stepRange)).* F2d(x) - stencil1Output), stepRange, stepRange.^3, 'LineWidth', 1.5)
xlabel("log(h)")
ylabel("Error")
...
figure
plot(stepRange,stencil2Output, stepRange, ones(size(stepRange)).* F2d(x), 'LineWidth', 1.5)
ylabel("f''(pi/2)")
xlabel("step size")
legend("FDM", "Analytical", "Location", "NorthWest")
axis([1e-2 0.1 -Inf Inf])
...
figure
loglog(stepRange, abs(ones(size(stepRange)).* F2d(x) - stencil2Output), stepRange, stepRange.^4, 'LineWidth', 1.5)
xlabel("log(h)")
ylabel("Error")
