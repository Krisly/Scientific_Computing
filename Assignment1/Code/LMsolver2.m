function [tout,yout] = LMsolver2( func, tspan, nsteps, y0, param ) 
...
% yout : vector of solution values corresponding to steps taken.
    ...
    for n = 1:(nsteps-1)
        ...
        % subsequent steps, normal operation
            ...
            %yout(n+1) = yout(n-1) + 1/3*h*func(tout(n-1), yout(n-1), param) + 4/3*h*func(tout(n), yout(n), param) + 1/3*h* func(tout(n+1), tout(n+1).^4 + 2.75681*tout(n+1).^2 + 1.9, param);
